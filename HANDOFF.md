...
## Added 2026-07-11 — security hardening, practice loop, docs

**Security (see `SECURITY.md` for the full write-up):**
- Proxy (`server/index.mjs`) rewritten: shared `x-app-key` gate on all `/api/`
  routes, model allowlist + `max_tokens` clamp (it can no longer be used as a
  free gateway to a pricier model), body-size caps with early abort, input
  validation, upstream errors summarised instead of forwarded, security headers,
  optional `FORCE_HTTPS`. Dead `/api/tts` route removed (ElevenLabs is gone).
  Requires `APP_KEY` on the server and `EXPO_PUBLIC_APP_KEY` in the app.
- `firebase.rules.json` — real database rules to replace test mode (validates
  shapes/types/sizes; note in-file that Anonymous Auth is the upgrade for true
  per-user write locks). **Not yet pasted into Firebase Console.**
- `npm audit`: 25 findings, all transitive deps of Expo SDK 54 itself.
  Deliberately NOT fixed — `audit fix --force` breaks Expo Go compatibility.
- Git history verified clean of secrets.

**Practice loop (the CAC differentiator):**
- `src/services/progress.ts` — analyses saved debates into per-skill averages,
  trends, and a weakest skill (owner's turns only, `MIN_SAMPLES` guard).
  Also `weakestSkillOf(session)` for a single debate.
- `src/app/progress.tsx` — skill dashboard with bars, trend arrows, and a
  "Practice this weakness" CTA.
- `src/app/practice.tsx` — a single AI-generated drill for one skill, graded on
  that skill alone (score / what worked / what to change / a stronger rewrite).
- `PRACTICE_DRILL_PROMPT` + `PRACTICE_FEEDBACK_PROMPT` in prompts.ts;
  `getPracticeDrill()` + `scorePracticeAttempt()` in claude.ts.
- Wired: home footer "Progress", results screen "Practice your <weak skill>".
- Routes registered in `_layout.tsx`. NOTE: Expo Router's typed routes live in
  `.expo/types/router.d.ts` and are only regenerated by the **dev server** —
  after adding a screen, boot `expo start` once or tsc will reject the new path.

**Home screen:** added a personal "your form" strip (debates / best score /
what you're working on) that links to the dashboard; retagged the hero line to
"Say your piece. Get judged. Settle it." (the old one leaked "Claude judges").

**Docs:** `CODE_TOUR.md` (file map, how a debate flows through the code, the
parts worth showing a judge) and `SECURITY.md`.
